...
const Contract = () => {
  ...
  // lấy data từ api    
  useEffect(() => {
    const departmentContractData  = JSON.parse (localStorage.getItem("departmentContractData"));
    if (departmentContractData  && departmentContractData.length !== 0) {
      setData(departmentContractData );
    } else {
      setData(ContractData );
      localStorage.setItem("departmentContractData", JSON.stringify(ContractData ));
    }
  }, []);
  const handleAdd = (record, values) => {
    ...
    localStorage.setItem("departmentContractData", JSON.stringify([...data, newItem]));
    setCustomerName("");
    setProduct("");
    setTotalAmount("");
  ...
  return (
    <div>
      <Space size={20} direction="vertical">
        ...
        <Space size="middle">
        <Input value={customerName} onChange={handleInputCustomerName} placeholder="Họ & Tên"/>
        <Input value={product} onChange={handleInputProduct} placeholder="Sản phẩm "/>
        <Input value={totalAmount} onChange={handleInputTotalAmount} placeholder="Giá trị" />
      </Space>
            ...
            <div>
              <Form>
                ...
                <Form.Item>
                  ...
                  <Input placeholder="Sản phẩm" value={product} onChange={(e) => setProduct(e.target.value)} />
                </Form.Item>
                <Form.Item>
                  <label htmlFor="">Giá trị</label>
                  <Input placeholder="Giá trị" value={totalAmount} onChange={(e) => setTotalAmount(e.target.value)} />
                </Form.Item>
              </Form>
            </div>
          )}
